...
def add_student(name: str, age: Optional[int] = None, email: Optional[str] = None) -> Optional[int]:
    ...
    try:
        ...
        log_message(create_log_message(f"Successfully added student '{name}' with ID {student_id}."))
        return student_id
    except sqlite3.Error as e:
...
def add_session_lesson(session_id: int, lesson_id: int, start_time: float, end_time: float, 
    ...
    try:
        session_lesson_data = prepare_session_lesson_data(
            session_id, lesson_id, start_time, end_time, total_questions, questions_correct
        )
        session_lesson_id = insert_session_lesson(session_lesson_data)
        log_successful_insertion(session_lesson_id)
        return session_lesson_id
    except sqlite3.Error as e:
        handle_session_lesson_error(e)
        return None


def prepare_session_lesson_data(session_id: int, lesson_id: int, start_time: float, end_time: float, 
                                total_questions: int, questions_correct: int) -> Tuple:
    """
    Prepare the data for inserting a session lesson record.
    ...
    Returns:
        Tuple: A tuple containing all the values needed for the database insertion.
    """
    ...
    return (session_id, lesson_id, start_time_str, end_time_str, total_time, 
            total_questions, questions_correct, avg_time_per_question, percent_correct)


def insert_session_lesson(data: Tuple) -> int:
    """
    Insert a session lesson record into the database.

    Parameters:
        data (Tuple): The data for the session lesson record.

    Returns:
        int: The ID of the newly inserted session lesson record.
    """
    connection, cursor = get_database_connection()
    try:
        cursor.execute('''
...
def log_successful_insertion(session_lesson_id: int):
    """
    Log the successful insertion of a session lesson record.

    Parameters:
        session_lesson_id (int): The ID of the newly inserted session lesson record.
    """
    log_entry = create_log_message(f"Session lesson record added with ID: {session_lesson_id}")
    log_message(log_entry)


def handle_session_lesson_error(error: sqlite3.Error):
    """
    Handle errors encountered during session lesson insertion.

    Parameters:
        error (sqlite3.Error): The database error that occurred.
    """
    log_entry = create_log_message(f"Error adding session lesson record: {error}")
    log_message(log_entry)
...
def bonus_game_fat_tuna():
    ...
    while not controls_displayed:
        ...
        draw_text("W/Space/Up = Jump", font, text_color, 0, HEIGHT * 0.4, center=True, enable_shadow=True)
        draw_text("A/Left = Move Left", font, text_color, 0, HEIGHT * 0.6, center=True, enable_shadow=True)
        draw_text("D/Right = Move Right", font, text_color, 0, HEIGHT * 0.8, center=True, enable_shadow=True)
        ...
        while running:
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if event.key == pygame.K_SPACE or event.key == pygame.K_w or event.key == pygame.K_UP or event.key == pygame.K_KP8:
                        cat.jump()
            ...
            while running:
                ...
                for event in pygame.event.get():
                    if event.type == pygame.QUIT:
                        ...
                        if event.key in [pygame.K_SPACE, pygame.K_w, pygame.K_UP, pygame.K_KP8]:
                            cat.jump()
...
def bonus_game_cat_pong():
    ...
    try:
        while running:
            ...
            if (keys[pygame.K_w] or keys[pygame.K_UP]) and player_cat_rect.top > 0:
                player_cat_rect.y -= PLAYER_SPEED
            if (keys[pygame.K_s] or keys[pygame.K_DOWN]) and player_cat_rect.bottom < HEIGHT:
                player_cat_rect.y += PLAYER_SPEED
                
            # Joystick controls (only if a joystick is connected)
            if joystick:
                # Left Stick Up/Down (Axis 1)
                axis_1_value = joystick.get_axis(1)  # Read vertical movement
            
                # Apply dead zone to prevent drift
                if abs(axis_1_value) > 0.2:
                    if axis_1_value < 0 and player_cat_rect.top > 0:  # Stick Up
                        player_cat_rect.y -= PLAYER_SPEED
                    elif axis_1_value > 0 and player_cat_rect.bottom < HEIGHT:  # Stick Down
                        player_cat_rect.y += PLAYER_SPEED
            
                # D-Pad Up/Down (Buttons 11 & 12)
                if joystick.get_numbuttons() > 12:  # Ensure it has enough buttons
                    if joystick.get_button(11) and player_cat_rect.top > 0:  # D-pad Up
                        player_cat_rect.y -= PLAYER_SPEED
                    if joystick.get_button(12) and player_cat_rect.bottom < HEIGHT:  # D-pad Down
                        player_cat_rect.y += PLAYER_SPEED
...
def random_quote_display():
    ...
    quotes = [
        ...
        ("He made an instrument to know /n If the moon shine at full or no.", "Samuel Butler"),
...
def generate_fraction_multiplication_problem():
    """Generates two fractions with single-digit numerators and denominators and returns the problem and the answer."""
    ...
    answer_numerator = numerator1 * numerator2
    answer_denominator = denominator * denominator

    return numerator1, numerator2, denominator, answer_numerator, answer_denominator


def display_fraction_multiplication_problem(numerator1, numerator2, denominator, user_input, first_input):
    screen.fill(screen_color)
    
    right_x = WIDTH * 0.55  # Right edge for alignment
    num1_y = HEIGHT * 0.35
    num2_y = HEIGHT * 0.5
    product_y = HEIGHT * 0.63
    ...
    multiply_sign_x = right_x - fraction1_surface.get_width() - WIDTH * 0.05
    multiply_surface = font.render("x", True, text_color)
    multiply_rect = multiply_surface.get_rect(right=multiply_sign_x, centery=num2_y)
    screen.blit(multiply_surface, multiply_rect)

    # Draw the second fraction
    fraction2_str = f"{numerator2}/{denominator}"
    fraction2_surface = font.render(fraction2_str, True, text_color)
    fraction2_rect = fraction2_surface.get_rect(right=right_x, centery=num2_y)
    screen.blit(fraction2_surface, fraction2_rect)

    # Draw the answer placeholder or user input
    input_text = "?" if first_input else user_input
    input_surface = font.render(input_text, True, text_color)
    input_rect = input_surface.get_rect(right=right_x, centery=product_y)
    screen.blit(input_surface, input_rect)

    pygame.display.flip()


def display_fraction_multiplication_explanation():
    """
    Display a multi-step explanation for multiplying fractions with the same denominator and why it's an important step.
    """
    explanation_lines = [
        "When multiplying fractions, multiply the numerators together and the denominators together.",
        "For example, (3/4) x (5/4) = (3x5) / (4x4) = 15/16.",
        "This lesson will help you practice multiplying fractions with the same denominator.",
        "Once you've mastered this, you'll be ready for more complex fraction problems!"
    ]
...
def fraction_multiplication_intro(session_id):
    """Displays an introduction screen for fraction multiplication, checking for a perfect score from the previous session."""
    global current_student  # Access the global current student
    
    # Check if the student got a perfect score on this lesson yesterday
    perfect_score_yesterday = perfect_score_lesson_skip(current_student, 'Fraction Multiplication')
    
    today = datetime.now().date()
    yesterday = today - timedelta(days=1)
    log_entry = create_log_message(
        f"Student: {current_student}, Lesson: 'Fraction Multiplication', "
        f"Date: {yesterday}, 100%: {'Yes' if perfect_score_yesterday else 'No'}"
    )
    log_message(log_entry)
    ...
    while waiting:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                mouse_pos = pygame.mouse.get_pos()
                if explanation_button_rect.collidepoint(mouse_pos):
                    display_fraction_multiplication_explanation()
                    waiting = False
                elif continue_rect.collidepoint(mouse_pos):
                    waiting = False
                elif skip_rect and skip_rect.collidepoint(mouse_pos):
    ...
    if skip_clicked:
        try:
            connection, cursor = get_database_cursor()
            current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            cursor.execute("SELECT lesson_id FROM lessons WHERE title = ?", ('Fraction Multiplication',))
            result = cursor.fetchone()
            if result:
                lesson_id = result[0]
                cursor.execute('''
    ...
    while waiting:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if continue_rect.collidepoint(event.pos):
                    waiting = False  # Proceed to the quiz
                    

def fraction_multiplication_quiz(session_id):
    """Presents a multiplication quiz of fractions with single-digit numerators and denominators."""
    intro_result = fraction_multiplication_intro(session_id)
    if intro_result == "skip":
        return 0, 0, 0  # Ensure the function returns three values when skipped
    """Presents a multiplication quiz of fractions with single-digit numerators and denominators."""
    if intro_result == "skip":
        return 0, 0, 0
    ...
    while problem_count < total_questions:
        numerator1, numerator2, denominator, answer_numerator, answer_denominator = generate_fraction_multiplication_problem()
        user_input = ""
        first_input = True
        question_complete = False
        ...
        while not question_complete:
            screen.fill(screen_color)
            display_fraction_multiplication_problem(numerator1, numerator2, denominator, user_input, first_input)
            pygame.display.flip()
                ...
                if event.type == pygame.QUIT:
                    ...
                    if (event.key == pygame.K_RETURN or event.key == pygame.K_KP_ENTER) and "/" in user_input:
                        ...
                        if num.isdigit() and denom.isdigit():
                            if int(num) == answer_numerator and int(denom) == answer_denominator:
                                correct_answers += 1
                                display_result("CORRECT!", "assets/images/cats")
                            else:
                                display_result(f"Sorry, the answer is {answer_numerator}/{answer_denominator}")
                            question_complete = True
                    elif event.key == pygame.K_BACKSPACE:
                        user_input = user_input[:-1]
                    elif event.unicode.isdigit() or (event.unicode == "/" and "/" not in user_input and len(user_input) > 0):
                        user_input += event.unicode
                    first_input = False
...
def generate_equivalent_fraction_problem(numerator_min, numerator_max, denominator_min, denominator_max):
    """Generates two fractions and returns their equivalent forms using the LCD."""
    numerator1 = random.randint(numerator_min, numerator_max)
    numerator2 = random.randint(numerator_min, numerator_max)
    denominator1 = random.randint(denominator_min, denominator_max)
    denominator2 = random.randint(denominator_min, denominator_max)
    ...
    lcd = math.lcm(denominator1, denominator2)
    converted_numerator1 = (numerator1 * (lcd // denominator1))
    converted_numerator2 = (numerator2 * (lcd // denominator2))
    
    return numerator1, denominator1, numerator2, denominator2, lcd, converted_numerator1, converted_numerator2


def display_equivalent_fraction_problem(numerator1, denominator1, numerator2, denominator2, user_input1, user_input2, first_input):
    """
    Display the fraction problem asking students to convert fractions to an equivalent form.
    """
    screen.fill(screen_color)

    # Define positions for fraction display
    center_x = WIDTH // 2
    num1_y = HEIGHT * 0.35
    num2_y = HEIGHT * 0.5
    line_y = HEIGHT * 0.57
    ...
    # Draw the first fraction
    fraction1_str = f"{numerator1}/{denominator1}"
    draw_text(fraction1_str, font, text_color, x=center_x, y=num1_y, center=True)

    # Draw the second fraction
    fraction2_str = f"{numerator2}/{denominator2}"
    draw_text(fraction2_str, font, text_color, x=center_x, y=num2_y, center=True)

    # Draw the input fields or placeholders
    input_text1 = "?" if first_input else str(user_input1)
    input_text2 = "?" if first_input else str(user_input2)
    draw_text(input_text1, font, text_color, x=center_x, y=sum_y - 30, center=True)
    draw_text(input_text2, font, text_color, x=center_x, y=sum_y + 30, center=True)

    pygame.display.flip()


def display_equivalent_fractions_explanation():
    """
    Display a multi-step explanation of why we multiply both the numerator and denominator
    to create an equivalent fraction.
    """
    # explanation_lines = [
    #     "An equivalent fraction has the same value, even though it looks different.",
    #     "To create an equivalent fraction, we multiply the numerator and denominator by the same number.",
    #     "This works because any number divided by itself is just 1 (e.g., 3/3 = 1).",
    #     "Multiplying by 1 doesn’t change the fraction’s value, just how it looks!",
    #     "Now that you understand equivalent fractions, let's practice!"
    # ]
    explanation_lines = [
        "A fraction is just a way to show part of something.",
        "Sometimes, we need to change how a fraction looks without changing its value.",
        "To do this, we multiply both the top and bottom number by the same thing.",
        "For example: \n 1/2 = 2/4 = 3/6 \n (We just multiply both numbers!)",
        "This works because any number divided by itself is just 1. (Like 2/2 = 1!)",
        "Multiplying by 1 doesn’t change the value, just how the fraction looks!",
        "Now let’s try making some equivalent fractions!"
    ]
...
def equivalent_fractions_quiz_intro(session_id):
    """
    Display the introductory screen for the Equivalent Fractions quiz with
    three clickable buttons: "Why Multiply Both?", "Continue", and "Skip" if applicable.
    """
    global current_student  # Access the global current student

    # Check if the student got a perfect score on this lesson recently
    perfect_score_yesterday = perfect_score_lesson_skip(current_student, 'Equivalent Fractions')
    
    # Log the result of the perfect score check
    today = datetime.now().date()
    yesterday = today - timedelta(days=1)
    log_entry = create_log_message(
        f"Student: {current_student}, Lesson: 'Equivalent Fractions', "
        f"Date: {yesterday}, 100%: {'Yes' if perfect_score_yesterday else 'No'}"
    )
    log_message(log_entry)
    ...
    # Draw the main instructional text
    draw_text(
        "Let's practice converting fractions to equivalent forms!",
        font,
        text_color,
        x=0,
        y=HEIGHT * 0.1,
    ...
    # Draw the "Equivalent forms?" clickable text
    explanation_button_rect = draw_text(
        "Equivalent forms?",
        font,
        text_color,
    ...
    while waiting:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                mouse_pos = pygame.mouse.get_pos()
                if explanation_button_rect.collidepoint(mouse_pos):
                    # Show explanation for why we multiply both numerator and denominator
                    display_equivalent_fractions_explanation()
                    waiting = False
                elif continue_button_rect.collidepoint(mouse_pos):
                    # Move on to the next part if "Continue" is clicked
                    waiting = False
    ...
    if skip_clicked:
        try:
            ...
            # Retrieve lesson ID for recording skip
            cursor.execute("SELECT lesson_id FROM lessons WHERE title = ?", ('Equivalent Fractions Quiz',))
            result = cursor.fetchone()
            if result:
                lesson_id = result[0]
                # Insert session lesson with NULL values for performance metrics
...
# def equivalent_fractions_quiz(session_id):
#     """Presents a quiz on converting fractions to their equivalent forms using the LCD."""
#     global current_student  # Access the global current student
    
#     # Display the introductory screen and get the user's choice to proceed or skip
#     intro_result = equivalent_fractions_quiz_intro(session_id)

#     if intro_result == "skip":
#         return 0, 0, None  # No questions asked, no correct answers, no time taken

#     # Retrieve the lesson_id for Equivalent Fractions Problems
#     connection, cursor = get_database_cursor()
#     cursor.execute("SELECT lesson_id FROM lessons WHERE title = ?", ('Equivalent Fractions',))
#     result = cursor.fetchone()
    
#     if result:
#         eq_fraction_lesson_id = result[0]
#     else:
#         log_entry = create_log_message("Equivalent Fractions lesson not found in the database.")
#         log_message(log_entry)
#         cursor.close()
#         connection.close()
#         return 0, 0, 0
...
#     while problem_count < total_questions:
#         numerator1, denominator1, numerator2, denominator2, lcd, converted_numerator1, converted_numerator2 = generate_equivalent_fraction_problem(1, 9, 2, 12)
#         user_input1 = ""
#         user_input2 = ""
#         first_input = True
#         question_complete = False
...
#             display_equivalent_fraction_problem(numerator1, denominator1, numerator2, denominator2, user_input1, user_input2, first_input)
...
#                         try:
#                             if int(user_input1) == converted_numerator1 and int(user_input2) == converted_numerator2:
#                                 correct_answers += 1
#                                 if time_taken < 3:
#                                     display_result("CORRECT!", "assets/images/fast_cats", use_lightning=True)
#                                 else:
#                                     display_result("CORRECT!", "assets/images/cats", use_lightning=False)
#                             else:
#                                 display_result(f"Sorry, the correct answers are {converted_numerator1} and {converted_numerator2}")
#                         except ValueError:
#                             display_result("Invalid input, please try again")
...
#     if completion_times:
#         draw_text(f"Average Time: {average_time} seconds", font, text_color, WIDTH // 2, HEIGHT * 0.6, center=True, enable_shadow=True)
...
def equivalent_fractions_quiz(session_id):
    ...
    while problem_count < total_questions:
        ...
        first_input = True
        question_complete = False
        input_stage = 1  # Track input stage (1 for first numerator, 2 for second numerator)
        ...
        while not question_complete:
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if event.key == pygame.K_RETURN or event.key == pygame.K_KP_ENTER:
                        if input_stage == 1 and user_input1:  # Move to second input
                            input_stage = 2
                        elif input_stage == 2 and user_input2:  # Validate both inputs
                            end_time = time.time()
                            time_taken = round(end_time - start_time, 1)
                            completion_times.append(time_taken)
                        ...
                        if input_stage == 2 and user_input2:
                            user_input2 = user_input2[:-1]
                        elif input_stage == 1 and user_input1:
                            user_input1 = user_input1[:-1]
                    elif event.unicode.isdigit():
                        if input_stage == 1 and len(user_input1) < 2:
                            user_input1 += event.unicode
                        elif input_stage == 2 and len(user_input2) < 2:
                            user_input2 += event.unicode
                        first_input = False
...
def session_manager():
    ...
    lessons_to_play = ["greet_student",                     #JP
                       ...
                       ### DEBUG TESTING ###
                       # "def john_14_15",
                       # "fraction_multiplication_quiz",       #Math
                       ...
                       # "basic_shapes_quiz",
                       # "equivalent_fractions_quiz",         #Math
    ...
    for lesson in lessons_to_play:
        ...
        ### Maths ###
        ...
        elif lesson == "fraction_multiplication_quiz":
            lesson_result = fraction_multiplication_quiz(session_id)
            ...
            if lesson_result is not None:  
                ...
                log_message("Error: fraction_multiplication_quiz did not return a valid result.")
        ...
        elif lesson == "equivalent_fractions_quiz":
            lesson_result = equivalent_fractions_quiz(session_id)
            ...
            if lesson_result is not None:  
                ...
                log_message("Error: equivalent_fractions_quiz did not return a valid result.")
...
def john_14_15():
    """Greets the student and introduces the Bible verse John 14:15 (NKJV)."""
    greeting_message = "It's time to work on a Bible verse!"
    verse_title = "John 14:15"
    verse_text = "If you love Me, keep My commandments."
    display_bible_verse(greeting_message, verse_title, verse_text)
...
class Cat:
    ...
    def handle_input(self):
        ...
        keys = pygame.key.get_pressed()
        if (keys[pygame.K_LEFT] or keys[pygame.K_a] or keys[pygame.K_KP4]) and self.rect.x > 0:
            self.rect.x -= self.speed
            if not self.facing_left:
                self.image = pygame.transform.flip(self.image, True, False)
                self.facing_left = True
        elif (keys[pygame.K_RIGHT] or keys[pygame.K_d] or keys[pygame.K_KP6]) and self.rect.x < WIDTH - self.rect.width:
            self.rect.x += self.speed
            if self.facing_left:
                self.image = pygame.transform.flip(self.image, True, False)
                self.facing_left = False
