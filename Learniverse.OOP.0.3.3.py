...
class MainMenuState(GameState):
    ...
    def __init__(self, 
        ...
        # Create the "Start" button and update it to switch to StudentSelectState
...
class StudentSelectState(GameState):
    ...
    def __init__(self, 
        ...
        # Initialize the input box for student name
        self.input_box = pygame.Rect(screen.get_width() // 2 - 100, screen.get_height() * 0.6, 200, 30)
        self.input_active = False
        self.input_text = ""

        # Initialize the "Add Student" button
        self.add_button = pygame.Rect(screen.get_width() // 2 - 50, screen.get_height() * 0.7, 100, 40)
    ...
    def handle_events(self, events: List[pygame.event.Event]) -> Union[str, None]:
        ...
        for event in events:
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                # If the user clicked the add button, add the student
    ...
    def draw(self, screen: pygame.Surface) -> None:
        ...
        # Display the title
        self.text_renderer.render_text(screen, "Student Selection", screen.get_width() // 2, screen.get_height() * 0.1, centered=True)
        
        # Display the list of students
        y_position = screen.get_height() * 0.2
        for student_name in self.students:
            self.text_renderer.render_text(screen, student_name, screen.get_width() // 2, y_position, centered=True)
            y_position += 30  # Adjust spacing as needed
        
        # Display the "Student Name:" label
        self.text_renderer.render_text(screen, "Student Name:", screen.get_width() // 2 - 150, self.input_box.y - 5)
        
        # Display the input box
        pygame.draw.rect(screen, (255, 255, 255), self.input_box, 2 if self.input_active else 1)
        self.text_renderer.render_text(screen, self.input_text, self.input_box.x + 5, self.input_box.y + 5)
        
        # Display the "Add Student" button
        pygame.draw.rect(screen, (0, 128, 0), self.add_button)
        self.text_renderer.render_text(screen, "Add Student", self.add_button.centerx, self.add_button.centery, centered=True)
...
class StateManager:
    ...
    def __init__(self, 
                 ...
                 sound_manager: SoundManager):  # Default value parameters go last
...
class Game:
    ...
    def __init__(self, config: Config):
        ...
        # Get the screen from the window manager
        screen = self.window_manager.get_screen()  # This line ensures the screen is initialized before use
        ...
        self.state_manager = StateManager(
            initial_state=IntroState(self.text_renderer, self.sound_manager), 
                ...
                screen=screen,  # Pass the screen object here
