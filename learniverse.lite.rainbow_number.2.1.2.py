...
def log_message(message):
    """Logs a message to both the console and the error_log.txt file with a timestamp."""
...
def apply_theme(theme_name):
    ...
    if theme_name in color_themes:
        ...
        # log_message(f"Applied theme: {theme_name}")
    else:
...
def draw_text(
    ...
    # Set shadow_color to global shadow_color if not provided
...
def draw_exit_button():
    ...
    exit_rect = draw_text("X", exit_font, RED, x_position, y_position, enable_shadow=True, return_rect=True)
...
def check_exit_click(mouse_pos, exit_rect):
    ...
    if exit_rect.collidepoint(mouse_pos):
        # log_message("Exit button clicked.")
        pygame.quit()
        sys.exit()
...
def select_random_background(folder_path):
    ...
    try:
        ...
        # log_message(f"Selected background image: {image_path}")  # Log the selected image
        return image_path
...
def main_menu():
    ...
    while True:
        ...
        draw_text("Learniverse: Rainbow Numbers", font, text_color, 0, HEIGHT * 0.2, center=True, enable_shadow=True, return_rect=False, max_width=WIDTH)
        start_rect = draw_text("Start", font, text_color, 0, HEIGHT * 0.55, center=True, enable_shadow=True, return_rect=True)
        options_rect = draw_text("Options", font, text_color, 0, HEIGHT * 0.7, center=True, enable_shadow=True, return_rect=True)
        explanation_rect = draw_text("Rainbow numbers?", font, text_color, 0, HEIGHT * 0.85, center=True, enable_shadow=True, return_rect=True)
        ...
        for event in pygame.event.get():
            ...
            #         # log_message("DEBUG: Skipping to Bonus Mode")  # Log for debugging
            #         return "bonus_game"  # Skip directly to the bonus game
...
def options_menu():
    ...
    while True:
        ...
        back_to_main_menu_rect = draw_text("Back to Main Menu", font, text_color, 0, back_to_main_y, center=True, enable_shadow=True, return_rect=True)

        # Draw the volume label, percentage, and control buttons with shadows
        draw_text("Volume:", font, text_color, WIDTH * (left_buffer * 2), volume_y, enable_shadow=True)
        draw_text(f"{int(music_volume * 100)}%", font, text_color, WIDTH * 0.5, volume_y, enable_shadow=True)
        
        # Calculate rects for volume control buttons based on the size of the rendered text
        minus_rect = draw_text("<", font, text_color, WIDTH * left_buffer, volume_y, enable_shadow=True, return_rect=True)
        plus_rect = draw_text(">", font, text_color, WIDTH * right_buffer, volume_y, enable_shadow=True, return_rect=True)

        # Draw the resolution controls with shadows
        draw_text("Resolution:", font, text_color, WIDTH * (left_buffer * 2), resolution_y, enable_shadow=True)
        resolution_text = f"{AVAILABLE_RESOLUTIONS[current_resolution_index][0]}x{AVAILABLE_RESOLUTIONS[current_resolution_index][1]}"
        draw_text(resolution_text, font, text_color, WIDTH * 0.5, resolution_y, enable_shadow=True)
        
        # Calculate rects for resolution control buttons
        resolution_minus_rect = draw_text("<", font, text_color, WIDTH * left_buffer, resolution_y, enable_shadow=True, return_rect=True)
        resolution_plus_rect = draw_text(">", font, text_color, WIDTH * right_buffer, resolution_y, enable_shadow=True, return_rect=True)

        # Draw the theme controls with shadows
        draw_text("Theme:", font, text_color, WIDTH * (left_buffer * 2), theme_y, enable_shadow=True)
        theme_name = list(color_themes.keys())[current_theme_index]
        draw_text(f"{theme_name.capitalize()}", font, text_color, WIDTH * 0.5, theme_y, enable_shadow=True)
        
        # Calculate rects for theme control buttons
        theme_minus_rect = draw_text("<", font, text_color, WIDTH * left_buffer, theme_y, enable_shadow=True, return_rect=True)
        theme_plus_rect = draw_text(">", font, text_color, WIDTH * right_buffer, theme_y, enable_shadow=True, return_rect=True)

        # Display the current font in use with < and > buttons
        left_arrow_rect = draw_text("<", font, text_color, WIDTH * left_buffer, font_y, enable_shadow=True, return_rect=True)
        draw_text(f"Font: {current_font_name_or_path}", font, text_color, WIDTH * 0.5, font_y, center=True, enable_shadow=True)
        right_arrow_rect = draw_text(">", font, text_color, WIDTH * right_buffer, font_y, enable_shadow=True, return_rect=True)

        # Draw the "Credits" option
        credits_rect = draw_text("Credits", font, text_color, 0, credits_y, center=True, enable_shadow=True, return_rect=True)
...
def save_options():
    ...
    try:
        ...
        # log_message("Options saved successfully.")
    except Exception as e:
...
def start_game():
    ...
    if completion_times:
        ...
        # log_message(f"Average time per question: {average_time} seconds")
...
def load_mp3(mp3):
    try:
        ...
        # log_message(f"{mp3} loaded successfully.")
...
def play_mp3(volume=None):
    ...
    try:
        ...
        # log_message(f"Playing music at {music_volume * 100}% volume.")
    except pygame.error as e:
...
def increase_volume(step=0.1):
    ...
    pygame.mixer.music.set_volume(music_volume)
    # log_message(f"Volume increased to {music_volume * 100}%.")

def decrease_volume(step=0.1):
    global music_volume
    music_volume = max(0.0, music_volume - step)  # Floor volume at 0.0 (mute)
    pygame.mixer.music.set_volume(music_volume)
    # log_message(f"Volume decreased to {music_volume * 100}%.")
