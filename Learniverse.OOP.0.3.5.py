...
class StudentSelectState(GameState):
    ...
    def __init__(self, 
        ...
        # Create a ButtonManager to manage all buttons
        self.button_manager = ButtonManager()

        # Create and add the "Add Student" button using the Button class
    ...
    def create_add_student_button(self):
        """Create the 'Add Student' button and add it to the ButtonManager."""
        screen_width = self.screen.get_width()
        screen_height = self.screen.get_height()

        # Calculate position for the add button
        add_button_x = screen_width // 2
        add_button_y = int(screen_height * self.ADD_BUTTON_Y_RATIO)

        # Create the button with text, and pass the add_student method as the action
        add_student_button = Button(
            x=add_button_x,
            y=add_button_y,
            text="Add Student",
            text_renderer=self.text_renderer,
            action=self.add_student  # The action to perform when button is clicked
        )
