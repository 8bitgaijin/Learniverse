...
class FontManager:
    ...
    def load_available_fonts(self) -> List[str]:
        ...
        # Debug output: look for VIVALDII.TTF specifically
        if 'VIVALDII.TTF' in font_files:
            print("VIVALDII.TTF found in font folder.")
        else:
            print("VIVALDII.TTF not found in font folder.")
        
        
        filtered_fonts = [f for f in font_files if not any(exclude in f.lower() for exclude in self.excluded_fonts)]
        
        # Debug print to list available and filtered fonts
        print("Available fonts before filtering:", font_files)  
        print("Filtered fonts:", filtered_fonts)  
...
class MainMenuState(GameState):
    ...
    def __init__(self, config: Config, text_renderer: TextRenderer, 
        ...
        self.start_button = Button(
            ...
            y=self.config.SCREEN_HEIGHT * 0.4,  # Y position
            text="Start Game",  # Text for the button
            text_renderer=text_renderer,  # TextRenderer object
            action=switch_to_gameplay  # Button action to start the game
        )
...
class OptionsMenuState(GameState):
    ...
    def __init__(self, window_manager: WindowManager, text_renderer: TextRenderer, 
                 font_manager: FontManager, switch_to_main_menu: callable, 
        ...
        # Define positions for resolution and font rows
        self.resolution_row_y = self.window_manager.config.SCREEN_HEIGHT * 0.2
        self.font_row_y = self.window_manager.config.SCREEN_HEIGHT * 0.4  # Position for font selection
    ...
    def apply_font_change(self):
        ...
        self.switch_to_options()  # Refresh the options menu to reflect font change
    ...
    def update_button_positions(self, screen: pygame.Surface) -> None:
        ...
        self.left_res_button.rect.x = int(screen_width * 0.2 - self.left_res_button.rect.width // 2)
        self.right_res_button.rect.x = int(screen_width * 0.8 - self.right_res_button.rect.width // 2)

        # Reposition font buttons based on the current screen size
        self.left_font_button.rect.x = int(screen_width * 0.2 - self.left_font_button.rect.width // 2)
        self.right_font_button.rect.x = int(screen_width * 0.8 - self.right_font_button.rect.width // 2)
        ...
        self.main_menu_button.rect.y = screen_height * 0.85  # Position the main menu button
        self.credits_button.rect.x = int(screen_width * 0.5 - self.credits_button.rect.width // 2)
        self.credits_button.rect.y = screen_height * 0.70  # Position the credits button
...
class GamePlayState(GameState):
    ...
    def __init__(self, text_renderer: TextRenderer, window_manager: WindowManager, input_manager: InputManager):
...
class StateManager:
    ...
    def switch_state(self, new_state_name: str) -> None:
        ...
        if new_state_name == "MAIN_MENU":
            self.current_state = MainMenuState(self.config, self.text_renderer, self.switch_to_options, self.switch_to_gameplay)
            ...
            self.current_state = GamePlayState(self.text_renderer, self.window_manager, self.input_manager)
        elif new_state_name == "OPTIONS_MENU":
            self.current_state = OptionsMenuState(self.window_manager, self.text_renderer, self.font_manager, self.switch_to_main_menu, self.switch_to_credits, self.switch_to_options)
...
class SoundManager:
    ...
    def load_random_mp3(self) -> None:
        """Load a random MP3 file from the specified music folder."""
        ...
        self.current_music = os.path.join(self.music_folder, random_file)
